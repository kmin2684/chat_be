    ...
    def serialize(self, user, mode):
        ...
        if mode == 'brief':
            if len(messages) > 0:
                ...
                last_message = {
                    'time': last_message.time_sent.strftime('%Y-%m-%dT%H:%M:%SZ'),
...
class User(AbstractUser):
    ...
    chat_room = models.ManyToManyField(
        Room, related_name="members", blank=True)
    ...
    def send_message(self, room, content):
        ...
        for member in members:
            if member == self:
                MessageCopy(original=message, owner=member,
                            checked=True).save()
            else:
    ...
    def enter_room(self, room):
        ...
        for message in messages:
            copies = MessageCopy.objects.filter(
                original=message, owner=self, checked=False)
            for copy in copies:
                copy.checked = True
                copy.save()
...
class Message(models.Model):
    ...
    def serialize(self):
        return {
            ...
            'time': self.time_sent.strftime('%Y-%m-%dT%H:%M:%SZ'),
            'room_id': self.room.id,
        }

    def __str__(self):
        return f"contnet: {self.content}, sender: {self.sender.username}, time: {self.time_sent.strftime('%Y-%m-%dT%H:%M:%SZ')}"


class MessageCopy(models.Model):
    original = models.ForeignKey(
        Message, on_delete=models.CASCADE, related_name="original_of")
